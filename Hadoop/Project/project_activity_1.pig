-- Load data from HDFS
inputDialogues4 = LOAD 'hdfs:///user/harshitha/inputs/episodeIV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues5 = LOAD 'hdfs:///user/harshitha/inputs/episodeV_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
inputDialogues6 = LOAD 'hdfs:///user/harshitha/inputs/episodeVI_dialogues.txt' USING PigStorage('\t') AS (name:chararray, line:chararray);
...
rmf hdfs:///user/harshitha/outputs;

-- Store result in HDFS
STORE namesOrdered INTO 'hdfs:///user/harshitha/outputs' USING PigStorage('\t');